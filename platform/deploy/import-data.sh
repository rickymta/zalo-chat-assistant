...
# GHI ĐÈ database zca và các thư mục bản phát hành có trong gói — dùng khi máy chủ còn trống hoặc cố ý thay toàn bộ.
...
if [ -f "$TMP/releases.dump" ]; then
  docker cp "$TMP/releases.dump" zca-mongo:/tmp/releases.dump
  docker exec zca-mongo mongorestore --quiet --gzip --archive=/tmp/releases.dump --drop --nsInclude='zca.releases'
  docker exec zca-mongo rm -f /tmp/releases.dump
fi
docker exec zca-mongo rm -f /tmp/zca.dump
echo "→ tệp cài + ảnh vào volume /data"
